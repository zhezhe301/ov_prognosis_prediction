...
for (k in 1:100) {
  ...
  predict <- survfit(coxph.model,newdata= as.data.frame(new.data.test[,intersectname]))   
  xxx <- summary(predict)
  predictval <- xxx$table[,'median']
  cindex1 <- rcorr.cens(predictval, Surv(new.data.test$days, new.data.test$vital_status))["C Index"]
  if (cindex1 >= 0.5) {
    mat.100[k,1] <- cindex1
  } else {
    mat.100[k,1] <- 1 - cindex1
  }
