		...
		for n in range(1,6):
			...
			def dnnclassifier():
				tf.logging.set_verbosity(tf.logging.INFO)
			 	traindata = pd.read_csv("~/Project/ov_prognosis_drugprecision/result/4.prediction/1.DNAseq/classifier.trainset.pca.5fold" + str(n) +".csv")
			 	y_train = traindata['txoutcome']
			 	X_train = traindata[list(range(2,len(traindata.columns)))]
			 	testdata = pd.read_csv("~/Project/ov_prognosis_drugprecision/result/4.prediction/1.DNAseq/classifier.testset.pca.5fold" + str(n) +".csv")
			 	...
			 	fout = open("/Users/Zhangzhe/Project/ov_prognosis_drugprecision/result/4.prediction/1.DNAseq/dnn_classifier/dnn_classifier.txoutcome.pca.5fold"+str(5*k+n)+".txt","w")
			...
			def treeclassifier():
				...
				print(y_test)
			 	# save the predicted value for the next step of C-index calculation by R				
				fout = open("/Users/Zhangzhe/Project/ov_prognosis_drugprecision/result/4.prediction/1.DNAseq/dt_classifier/dt_classifier.txoutcome.pca.5fold"+str(5*k+n)+".txt","w")
			...
			def rfclassifier():
				...
				rf_prediction = rf_classifier.predict(X_test)
			 	# save the predicted value for the next step of C-index calculation by R
				fout = open("/Users/Zhangzhe/Project/ov_prognosis_drugprecision/result/4.prediction/1.DNAseq/rf_classifier/rf_classifier.txoutcome.pca.5fold"+str(5*k+n)+".txt","w")
