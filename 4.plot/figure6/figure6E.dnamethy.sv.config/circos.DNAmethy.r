...
setwd('~/Project/ov_prognosis_drugprecision/result/2.molecular_data/')
total <- read.table('coxDNAmethy.txt',header=T,row.names = 1)
p2.1 <- row.names(total)[which(total[,1]<0.05)]
setwd('~/Project/ov_prognosis_drugprecision/result/7.circos//')
table.go2gene <- read.csv('1DNAmethy.survival.csv',stringsAsFactors= F)
setwd('~/Project/ov_prognosis_drugprecision/plot/circos/')
table.gofamily <- read.csv('goid.new.csv',stringsAsFactors= F)
#table.allgo <- read.csv('goid2genes.csv')
list.go <- table.go2gene[which(table.go2gene[,6]< 0.05), 1]

#### BP ####
table <- matrix(NA, ncol = 11,nrow = length(list.go))
table[,1] <- list.go
for (i in 1:length(list.go)) {
  if (is.na(table.gofamily[which(table.gofamily[,2]==list.go[i]),3])) {
...
setwd('~/Project/ov_prognosis_drugprecision/plot/circos/dnamethy.sv.config/')
write.table(karyotype,'karyotype.dnamethy.txt',row.names=F,col.names=F,quote= F,sep=' ')
