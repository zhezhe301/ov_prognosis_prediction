...
setwd('~/Project/ov_prognosis_drugprecision/result/8.plot_kmcurve/')
setEPS()
postscript('rnaseq.lasso.5fold.eps', width=9, height=6)
...
setwd('~/Project/ov_prognosis_drugprecision/result/8.plot_kmcurve/')
setEPS()
postscript('rnaseq.rf.5fold.eps', width=9, height=6)
table.curve.all[,4] <- as.factor(table.curve.all[,4])
kmfit <- survfit(Surv(days,vital_status)~table.curve.all[,4], data=table.curve.all, conf.type = "log-log")
