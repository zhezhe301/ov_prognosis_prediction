...
# write.csv(pmat, 'miRNAseq_pvalue.csv')
setwd('~/Project/ov_prognosis_drugprecision/result/5.tx.pvalue/')
list1 <- read.csv('miRNASeq_pvalue.csv',row.names = 1)
list1<- as.matrix(list1)
list2 <- sort(list1[,1])
list3 <- list2[1:123]  # <0.05, event :300
...
for (k in 1:nrFolds) {
  ...
  setwd('~/Project/ov_prognosis_drugprecision/result/4.prediction/3.miRNAseq/')
...
#summary(pca)
## 1-38:80%, 1-44:85%, 1-51:90%, 1-6 :95%
pca.table <- table1 %*% pca$loadings[,1:38]
...
for (k in 1:100) {
  setwd('~/Project/ov_prognosis_drugprecision/result/4.prediction/3.miRNAseq/dt_classifier/')
  ...
  setwd('~/Project/ov_prognosis_drugprecision/result/4.prediction/3.miRNAseq/rf_classifier/')
  ...
  setwd('~/Project/ov_prognosis_drugprecision/result/4.prediction/3.miRNAseq/dnn_classifier/')
