		...
		for n in range(1,6):
			...
			def dnnclassifier():
				tf.logging.set_verbosity(tf.logging.INFO)
			 	traindata = pd.read_csv("~/Project/ov_prognosis_drugprecision/result/4.prediction/2.RNAseq/classifier.trainset.5fold" + str(n) +".csv")
			 	y_train = traindata['txoutcome']
			 	X_train = traindata[list(range(2,len(traindata.columns)))]
			 	testdata = pd.read_csv("~/Project/ov_prognosis_drugprecision/result/4.prediction/2.RNAseq/classifier.testset.5fold" + str(n) +".csv")
				...
				# save the predicted value for the next step of C-index calculation by R
			 	fout = open("/Users/Zhangzhe/Project/ov_prognosis_drugprecision/result/4.prediction/2.RNAseq/dnn_classifier/dnn_classifier.txoutcome.5fold"+str(5*k+n)+".txt","w")
			...
			def treeclassifier():
				...
				print(y_test)
				# save the predicted value for the next step of C-index calculation by R	
				fout = open("/Users/Zhangzhe/Project/ov_prognosis_drugprecision/result/4.prediction/2.RNAseq/dt_classifier/dt_classifier.txoutcome.5fold"+str(5*k+n)+".txt","w")
			...
			def rfclassifier():
				...
				# save the predicted value for the next step of C-index calculation by R
				fout = open("/Users/Zhangzhe/Project/ov_prognosis_drugprecision/result/4.prediction/2.RNAseq/rf_classifier/rf_classifier.txoutcome.5fold"+str(5*k+n)+".txt","w")
