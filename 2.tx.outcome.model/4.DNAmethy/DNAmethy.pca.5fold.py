		...
		for n in range(1,6):
			...
			def dnnclassifier():
				tf.logging.set_verbosity(tf.logging.INFO)
			 	traindata = pd.read_csv("./classifier.trainset.pca.5fold" + str(n) +".csv")
			 	y_train = traindata['txoutcome']
			 	X_train = traindata[list(range(2,len(traindata.columns)))]
			 	testdata = pd.read_csv("./classifier.testset.pca.5fold" + str(n) +".csv")
				...
				# save the predicted value for the next step of C-index calculation by R			 	
			 	fout = open("./dnn_classifier.txoutcome.pca.5fold"+str(5*k+n)+".txt","w")
			...
			def treeclassifier():
				...
				print(y_test)
				# save the predicted value for the next step of C-index calculation by R	
				fout = open("./dt_classifier.txoutcome.pca.5fold"+str(5*k+n)+".txt","w")
			...
			def rfclassifier():
				...
				# save the predicted value for the next step of C-index calculation by R
				fout = open("./rf_classifier.txoutcome.pca.5fold"+str(5*k+n)+".txt","w")
